...
SnackShack.prototype.buy = function(quantity=1) {
  ...
  this.calculateSchedule(quantity) : 
...
SnackShack.prototype.calculateSchedule = function(quantity) {
  let schedule = `0:00 ${quantity} sandwich order` + (quantity>1 ? 's' : '') + ` placed, start making sandwich 1\n1:00 serve sandwich 1`
  for(i=2;i<=quantity;i++) {
    schedule += `\n${this.formatTime(i*1.5/2)} start making sandwich ${i}\n${this.formatTime(i*1.5/2+1)} serve sandwich ${i}`
  }
